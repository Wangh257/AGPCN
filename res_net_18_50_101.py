...
class Resnet_Unet_18_50_101(nn.Module):
    """
    定稿使用resnet50作为backbone

    BN_enable控制是否存在BN,定稿设置为True
    """
    def __init__(self, backbone='resnet101',BN_enable=True, resnet_pretrain=True):
        super().__init__()
        self.BN_enable = BN_enable
        # encoder部分
        ...
        if backbone=='resnet34':
            ...
            filters=[64,256,512,1024,2048]
        self.firstconv = nn.Conv2d(in_channels=4, out_channels=64, kernel_size=7, stride=2, padding=3, bias=False)
    ...
    def forward(self,x):
        ...
        return self.final(d4), d2

class Resnet_Unet_18_50_101_attention(Resnet_Unet_18_50_101):
    def __init__(self, backbone='resnet101', BN_enable=True, resnet_pretrain=True):
        super().__init__(backbone, BN_enable, resnet_pretrain)

        self.attention1 = SpatialAttention_WH(in_channel=64, inter_channel=64, subsample_rate=2)
